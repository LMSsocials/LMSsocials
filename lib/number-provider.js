...
import { FIXED_NUMBER_OFFER, numberSellingPriceKobo } from './number-pricing.js'
import { isNumberCountryEnabled } from './number-catalog.js'
...
export async function createNumberQuote({ userId, countryId, serviceCode, serverId }) {
  ...
  const fixedOffer = serverId === FIXED_NUMBER_OFFER.serverId && countryId === FIXED_NUMBER_OFFER.countryId && serviceCode === FIXED_NUMBER_OFFER.serviceCode
  const supplier = chooseNumberSupplier({ prices: priceData[countryId]?.[serviceCode], goldPartners, serverId, providerId: fixedOffer ? FIXED_NUMBER_OFFER.providerId : undefined })
