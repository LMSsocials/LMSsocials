...
import { isNumberCountryEnabled, numberProviderForOffer } from './number-catalog.js'
...
export function chooseNumberSupplier({ prices, goldPartners, serverId, providerId }) {
  ...
  if (providerId) return candidates.find((row) => row.providerId === String(providerId)) || null
...
export async function createNumberQuote({ userId, countryId, serviceCode, serverId }) {
  if (!isNumberCountryEnabled(countryId)) throw new NumberProviderError('UNAVAILABLE', true)
  ...
  const requiredProviderId = numberProviderForOffer({ countryId, serviceCode, serverId })
  const supplier = chooseNumberSupplier({ prices: priceData[countryId]?.[serviceCode], goldPartners, serverId, providerId: requiredProviderId })
  if (!supplier) throw new NumberProviderError(serverId === '3' ? 'NO_GOLD_NUMBERS' : 'NO_NUMBERS', true)
  const configuredQuality = requiredProviderId ? { '1': 'standard', '2': 'silver', '3': 'gold' }[serverId] : null
  const quote = {
    userId: String(userId), countryId, serviceCode, serverId, providerId: supplier.providerId,
    providerPriceUsd: supplier.price, sellingPriceKobo: numberSellingPriceKobo(supplier.price, serverId, { countryId, serviceCode, providerId: supplier.providerId }),
    quality: configuredQuality || (supplier.gold ? 'gold' : 'standard'), available: supplier.available,
...
export async function verifyNumberQuote(token, userId) {
  ...
  if (payload.sellingPriceKobo !== numberSellingPriceKobo(payload.providerPriceUsd, payload.serverId, payload)) {
