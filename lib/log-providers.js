...
const roundKobo = (value) => Math.ceil(value / 5000) * 5000
...
function categoryFor(...values) {
  ...
  if (text.includes('gmail') || text.includes('google')) return 'Google Voice'
...
function sujanPriceKobo(priceMinor, quantity = 1) {
  const markup = 1 + (number(process.env.SUJAN_MARKUP_PERCENT) || 25) / 100
...
export async function bulkCatalog() {
  ...
  const items = [first, ...rest].flatMap((payload) => payload.data?.items || []).filter((item) => number(item.inStock) > 0).map((item) => {
    ...
    return {
      _id: `bulkacc:${item.code}`, source: 'bulkacc', title: cleanText(item.name, 110), brand: categoryFor(item.groupName, item.categoryName, item.name),
      category: categoryFor(item.groupName, item.categoryName, item.name), description: cleanText(item.description || item.categoryName || 'Instant digital delivery.'),
...
export async function sujanCatalog() {
  ...
  const items = (payload.data || []).filter((item) => number(item.available_stock) > 0).map((item) => {
    const category = categoryFor(item.category?.name, item.platform?.name, item.name)
    return {
