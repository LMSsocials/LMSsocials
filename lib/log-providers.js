...
function bulkPriceKobo(price, quantity = 1, percentage) {
  const rate = number(process.env.BULKACC_USD_TO_NGN_RATE) || 1600
  const markup = 1 + markupPercent(percentage, 'BULKACC_MARKUP_PERCENT') / 100
...
export async function bulkCatalog(pricing = {}) {
  const activeMarkup = markupPercent(pricing.bulkaccMarkupPercent, 'BULKACC_MARKUP_PERCENT')
  const cached = globalThis.__bulkLogCatalog
  if (cached && cached.markupPercent === activeMarkup && Date.now() - cached.createdAt < CACHE_TTL) return cached.items
  ...
  const items = [first, ...rest].flatMap((payload) => payload.data?.items || []).filter((item) => number(item.inStock) > 0).map((item) => {
    ...
    return {
      ...
      priceKobo: bulkPriceKobo(item.price, minimum, activeMarkup), stockCount: Math.floor(number(item.inStock) / minimum), quantity: minimum, imageUrl: '',
    }
  })
  globalThis.__bulkLogCatalog = { createdAt: Date.now(), markupPercent: activeMarkup, items }
  return items
}

export async function bulkProduct(code, pricing = {}) {
  ...
  return { code: String(payload.data.code), title: payload.data.name, quantity: minimum, priceKobo: bulkPriceKobo(payload.data.price, minimum, pricing.bulkaccMarkupPercent) }
}
...
export async function sujanCatalog(pricing = {}) {
  ...
  const items = (payload.data || []).filter((item) => number(item.available_stock) > 0).map((item) => {
    ...
    return {
      ...
      description: cleanText(item.description || 'Instant digital delivery.'), priceKobo: sujanPriceKobo(item.price_minor, 1, activeMarkup),
      stockCount: number(item.available_stock), quantity: 1, imageUrl: '',
    }
  })
  globalThis.__sujanLogCatalog = { createdAt: Date.now(), markupPercent: activeMarkup, items }
  return items
}
