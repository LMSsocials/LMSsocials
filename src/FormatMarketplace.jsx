...
const formatType = (file) => file?.contentType === 'text/plain' || String(file?.fileName || '').toLowerCase().endsWith('.txt') ? 'TXT' : 'PDF'
...
export default function FormatMarketplace() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!filesResponse.ok) throw new Error(filesPayload.message || 'Unable to load formats')
  ...
  async function purchase(file) {
    ...
    try {
      ...
      setMessage('Purchase complete. Your file is ready to download.')
  ...
  return <section className='logs-market voucher-market format-market'>
    <header className='logs-head'>
      <div><span>FORMAT LIBRARY</span><h2>Downloadable formats.</h2><p>Browse the available PDF and text resources. Purchased files stay in your library for future downloads.</p></div>
      <div className='voucher-trust'><BookOpen /><span><strong>PDF &amp; TXT files</strong><small>Private access after purchase</small></span></div>
    </header>
    <div className='voucher-tools'><label><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder='Search formats...' /></label></div>
    {message && <div className='voucher-message'>{message}</div>}
    {state === 'loading' ? <div className='logs-empty'><LoaderCircle className='spin' /><strong>Loading formats</strong></div> : <div className='format-file-grid'>
      {visibleFiles.map((file) => {
        const order = purchased.get(file._id)
        return <article key={file._id}>
          <i><FileText /></i>
          <div><span>{formatType(file)} · {fileSize(file.fileSize)}</span><h3>{file.title}</h3><p>{file.description || file.fileName}</p></div>
          <strong>{money(file.priceKobo)}</strong>
          {order ? <a href={order.downloadUrl}><Download /> Download</a> : <button disabled={Boolean(buying)} onClick={() => purchase(file)}>{buying === file._id ? <LoaderCircle className='spin' /> : <ShoppingBag />} Buy {formatType(file)}</button>}
        </article>
      })}
      {!visibleFiles.length && <div className='logs-empty'><FileText /><strong>No formats available</strong><span>Published PDF and TXT uploads will appear here.</span></div>}
    </div>}
    {orders.length > 0 && <section className='format-library'>
      <div className='dash-section-title'><div><span>YOUR LIBRARY</span><h2>Purchased formats</h2></div><small>{orders.length}</small></div>
